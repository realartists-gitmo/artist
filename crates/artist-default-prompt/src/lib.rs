//! Default replaceable prompt-composition component.

wit_bindgen::generate!({
    path: "../artist-kernel/wasm/composition/wit",
    ...
    fn initial(input: SessionInput) -> Result<Snapshot, Error> {
        ...
        if let Some(content) = input.system {
            contributions.push(Contribution {
                id: "system".into(),
                source: Some("prompt://SYSTEM.md".into()),
                slot: "system".into(),
                order: 0,
                revision: 1,
                content,
        ...
        if !input.tool_events.is_empty() {
            contributions.push(Contribution {
                ...
                order: 0,
                revision: 1,
                content: input.tool_events.join("\n"),
            });
        }
        ...
        contributions.push(Contribution {
            ...
            order: 0,
            revision: 1,
            content: format!("You are {}.", input.identity),
        });
