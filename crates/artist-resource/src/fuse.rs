//! FUSE view of the logical resource tree (libfuse on Linux, macFUSE on macOS).
...
impl ResourceFs {
    fn new(router: ResourceRouter, runtime: Handle, mount: PathBuf) -> Self {
        Self {
            model: ResourceMountModel::new(router, runtime, mount),
            inodes: Mutex::new(Inodes {
                next: 2,
                nodes: HashMap::from([(1, MountNode::Root)]),
    ...
    fn node(&self, ino: INodeNo) -> Option<MountNode> {
        self.inodes.lock().ok()?.nodes.get(&u64::from(ino)).cloned()
    }
    fn inode(&self, key: String, node: MountNode) -> INodeNo {
    ...
    fn classify(&self, uri: ResourceUri) -> Option<(INodeNo, bool, u64)> {
        let entry = self.model.classify(uri)?;
        let key = match &entry.node {
            MountNode::Resource { uri, .. } => uri.to_string(),
            _ => return None,
        };
        let ino = self.inode(key, entry.node);
        Some((ino, entry.directory, entry.size))
    }
...
impl Filesystem for ResourceFs {
    fn lookup(&self, _: &Request, parent: INodeNo, name: &OsStr, reply: ReplyEntry) {
        ...
        if matches!(parent, MountNode::Root) {
            if !self.model.schemes().iter().any(|scheme| scheme == name) {
                reply.error(Errno::ENOENT);
                return;
            }
            let ino = self.inode(format!("scheme:{name}"), MountNode::Scheme(name.into()));
            self.acquire_lookup(ino);
            reply.entry(&TTL, &Self::attr(ino, true, 0), Generation(0));
            return;
        }
        let Some(uri) = self.model.child_uri(&parent, name) else {
            reply.error(Errno::ENOENT);
            return;
        };
    ...
    fn getattr(&self, _: &Request, ino: INodeNo, _: Option<FileHandle>, reply: ReplyAttr) {
        match self.node(ino) {
            Some(MountNode::Root | MountNode::Scheme(_)) => {
                reply.attr(&TTL, &Self::attr(ino, true, 0))
            }
            Some(MountNode::Resource { uri, directory }) => {
                if directory {
                    reply.attr(&TTL, &Self::attr(ino, true, 0));
                    return;
                }
    ...
    fn setattr(
        ...
        let mut bytes = match self.model.handle(ResourceRequest::Read {
            uri: uri.clone(),
            start_line: None,
            line_count: None,
        }) {
        ...
        match self.model.handle(ResourceRequest::Write { uri, text }) {
    ...
    fn open(&self, _: &Request, ino: INodeNo, _: OpenFlags, reply: ReplyOpen) {
        ...
        let bytes = match self.model.handle(ResourceRequest::Read {
            uri: uri.clone(),
            start_line: None,
            line_count: None,
        }) {
    ...
    fn create(
        ...
        match self.model.handle(ResourceRequest::Write {
            uri: uri.clone(),
            text: String::new(),
        }) {
            Ok(_) => {
    ...
    fn readdir(
        ...
        for entry in self.model.entries(&node) {
            let key = match &entry.node {
                MountNode::Root => "/".into(),
                MountNode::Scheme(scheme) => format!("scheme:{scheme}"),
                MountNode::Resource { uri, .. } => uri.to_string(),
            };
            let child = self.inode(key, entry.node);
            entries.push((
                child,
                if entry.directory {
                    FileType::Directory
    ...
    fn rename(
        ...
        let (Some(from), Some(to)) = (
            self.model.child_uri(&parent, name),
            self.model.child_uri(&newparent, newname),
        ) else {
            reply.error(Errno::EINVAL);
            return;
        };
...
impl ResourceFs {
    fn remove(&self, parent: INodeNo, name: &OsStr, reply: ReplyEmpty) {
        ...
        let Some(from) = self.model.child_uri(&parent, name) else {
            reply.error(Errno::EINVAL);
            return;
        };
        match self.model.handle(ResourceRequest::Move { from, to: None }) {
            Ok(_) => reply.ok(),
            Err(e) => reply.error(Self::error(e)),
        }
    }
