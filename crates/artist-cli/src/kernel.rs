...
pub fn address(target: &str) -> Result<ResourceAddress> {
    if target.contains("://") {
        return ResourceUri::parse(target)
            .map(ResourceAddress::Uri)
            .with_context(|| format!("parse resource URI {target:?}"));
    }
    if target.is_empty() {
        bail!("resource target cannot be empty");
    }
    Ok(ResourceAddress::path(target))
}
...
mod tests {
    ...
    #[test]
    fn distinguishes_native_paths_from_virtual_uris() {
        assert!(matches!(
            address("src/main.rs").unwrap(),
            ResourceAddress::Path(_)
        ));
        assert!(matches!(
            address("session://local/one").unwrap(),
            ResourceAddress::Uri(_)
        ));
    }
