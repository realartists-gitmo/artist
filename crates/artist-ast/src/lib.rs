...
//! render them.
//!
//! That last part matters here specifically. Renderers are deliberately not
//! part of this crate's contract. Analysis in, structured data out.
...
/// Parse an already-loaded UTF-8 source buffer using the path's language
/// adapter, without rereading the file from disk.
pub fn parse_source(path: &Path, source: &str) -> Option<ParseResult> {
    crate::main_helpers::parse_source_for_hook(path, source)
}
