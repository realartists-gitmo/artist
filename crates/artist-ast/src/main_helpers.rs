...
pub fn parse_file_for_hook(path: &Path) -> Option<ParseResult> {
    ...
    parse_source_for_hook(path, &source)
}

/// Parse an already-loaded UTF-8 source buffer without rereading the path.
/// Resource handlers use this so analysis and returned content come from the
/// same filesystem snapshot.
pub fn parse_source_for_hook(path: &Path, source: &str) -> Option<ParseResult> {
    ...
    let mut result = match lang {
        SupportLang::Rust => RustAdapter.parse(
            path,
            source.as_bytes(),
            lang.ast_grep(source.to_owned()).root(),
        ),
