...
mod tests {
    ...
    fn cache_root_and_model_dir_honour_env_override() {
        ...
        // SAFETY: edition 2024 makes the thread-safety contract explicit. The
        // hazard is another thread reading the environment concurrently; no
        // other test in this crate touches `AST_BRO_MODEL_DIR` (the two that
        // did were folded into this one upstream, see above), and nothing here
        // spawns threads between the set and the remove.
        unsafe { std::env::set_var("AST_BRO_MODEL_DIR", &path) };

        let resolved_root = cache_root().unwrap();
        let resolved_model = model_dir(&ModelInfo::potion_code_16m()).unwrap();

        unsafe { std::env::remove_var("AST_BRO_MODEL_DIR") };
    ...
    fn network_real_download() {
        let tmp = tempfile::tempdir().unwrap();
        // SAFETY: as above. This test is `#[ignore]`d and run alone.
        unsafe { std::env::set_var("AST_BRO_MODEL_DIR", tmp.path()) };
        let info = ModelInfo::potion_code_16m();
