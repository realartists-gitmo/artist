...
mod tests {
    ...
    #[test]
    fn window_tail_keeps_the_trailing_window_on_a_char_boundary() {
        assert_eq!(window_tail("abcdef", 100), "abcdef", "shorter than window");
        assert_eq!(window_tail("abcdef", 3), "def");
        // "界" is 3 bytes, so a 3-byte window lands exactly on its start.
        assert_eq!(window_tail("aa界", 3), "界");
        // A cut landing mid-codepoint snaps *forward*, dropping the partial
        // char, so the slice never exceeds the rule's window.
        assert_eq!(window_tail("aa界", 2), "");
        assert_eq!(window_tail("", 4), "");
    }
