...
**Continuation repair — stdin and custom-driver lifecycle.** Invocation-owned scopes now carry the receiver for the single public invocation's `/stdin` stream. A scalar named-tool execution attaches that receiver before dispatch; an interactive provider can await `InvocationScope::next_stdin()` and consume subsequent typed writes without creating a second invocation root. `InvocationStore` has regression coverage proving a write reaches an active consumer and remains in stdin history. The custom AgentRun driver also replays terminal assistant text/reasoning when a provider supplies only a final choice, emits completion-usage telemetry, and continues to demultiplex tool results through the authoritative lossless stdout projection. `stdobs` remains exactly equal to stdout by the current mandate.
...
**Latest repair clarification.** Package-owned `stdobs` is authoritative again: the host preserves the observer returned by the active component and model rendering consumes it, while the shipped default observers currently mirror stdout as requested. Dynamic catalog reconciliation is ownership-scoped; resource providers publish canonical universal input contracts and claim only their own identities; failed hot-swap candidates leave the prior executable/schema generation published; successful candidates update both atomically. Batched model calls now create per-slot invocation scopes with distinct public URIs and stdin subscriptions, process read/poll anchors share byte-offset cursors, invocation child responses name the addressed child URI, and terminal Rig replay tracks text and reasoning independently. The resources provider no longer claims every verb on `resources://`, preventing collisions with unrelated active identities. Verification has now passed for the workspace compilation, kernel tests, the component generation/observer tests, and the full workspace test run after these corrections; repository warning noise remains but no test failures remain.
...
**Latest audit repair.** The process provider now publishes the exact universal request shapes for read, poll, abort, and delete, including the URI where the universal adapter supplies it. Active component generations now own the registration metadata used for execution; execution does not combine a pinned old component with freshly reread disk metadata. Observer traps are non-authoritative: the typed stdout result survives, while only that item receives observer stderr/stdobs degradation and sibling items continue. Provider-batch requests carry their original invocation scopes through claim grouping, so per-call stdin and correlation remain aligned after regrouping.

Authoritative stdout, package-owned stdobs, and stderr are now independent channels. Publishing stdout cannot synthesize or overwrite stdobs, and an intentionally empty observer remains empty. Nested provider results are no longer published into the public invocation before the outer WASM adapter lifts them. Missing provider output contracts receive the canonical universal response contract, `run` includes its argument list in the canonical fallback request, and semantic `Result::Err` values are classified as tool errors in agent telemetry even when transport succeeded.

The implementation is ready for final verification. Per the mandate, no tests were run during implementation; compilation and diff checks were used only to finish source repair. The next verification pass is the first test pass after the full implementation, followed by recording any genuine failures here rather than treating them as intermediate milestones.

**Final verification for this repair.** After the implementation was complete, `cargo fmt --all -- --check`, `git diff --check`, and `cargo test --workspace --all-features` passed. The full workspace test run passed all unit, integration, and doc-test targets, including the 32-test CLI suite, 24 component tests, 48 kernel tests, dynamic resource/tool suites, and the remaining workspace suites. Existing compiler warning noise remains; there were no test failures.

**Current audit repair.** A model turn now captures one immutable `ToolDefinition` catalog, including package and generation, and carries that catalog through normalization, scope pinning, provider selection, and component execution. Published component generations retain the active executable and all derived contract/model metadata; execution and output decoding do not reparse mutable WIT paths. Older pinned names remain executable during a turn even if the current package is renamed.

Mutation barriers count logical participants rather than nested host calls. Reads do not register, duplicate mutation intent from one logical call conflicts, and each participant seals explicitly; a missing or excess mutation intent fails the transaction rather than hanging. Provider fallback output contracts now match the native values (raw read variants, filesystem hunks, and URI records), while semantic failure detection recognizes both transport errors and typed `Result::Err` stdout.

Observer scopes use the original logical batch slot. Programmatic scalar and batch entry points complete the same stdout/stderr/stdobs invocation lifecycle as model calls. Process polling uses the same byte-offset cursor for read and poll, returns post-cursor material, and applies matching only to that material; invocation polling treats terminal status as terminal even without a match. Invocation stdin writes store the request's content value, not the wrapper record.
