...
use crate::grep::{
    GrepResult, GrepSearchOptions, grep_search, grep_search_parsed, multi_grep_search,
};
...
impl FilePicker {
    ...
    /// Run a raw grep without parsing constraints or applying the legacy
    /// constrained-query literal fallback.  Consumers that own their pattern
    /// grammar can use this as the deterministic content-search primitive.
    pub fn grep_raw(&self, pattern: &str, options: &GrepSearchOptions) -> GrepResult<'_> {
        let query = FFFQuery {
            raw_query: pattern,
            constraints: Vec::new(),
            fuzzy_query: fff_query_parser::FuzzyQuery::Text(pattern),
            location: None,
        };
        let overlay_guard = self.sync_data.bigram_overlay.as_ref().map(|o| o.read());
        let arena = self.arena_base_ptr();
