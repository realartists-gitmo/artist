...
/// Pure fuzzy-line matching for consumers that do not own a `FilePicker`.
/// This uses the same scoring and acceptance policy as FFF filesystem grep.
pub fn fuzzy_line_matches(pattern: &str, line: &str) -> bool {
    ...
    let Some(indices) = matcher.match_list_indices(&[line]).into_iter().next() else {
        return false;
    };
    let needle_len = pattern.len();
    let perfect_score = (needle_len as u16) * 16;
    if indices.score < (perfect_score * 50) / 100 {
        return false;
    }
    if indices.indices.len() < needle_len.saturating_sub(max_typos).max(1) {
        return false;
    }
    ...
    let density = (indices.indices.len() * 100) / span;
    let minimum_density = if indices.indices.len() >= needle_len {
        45
    } else {
        65
    };
    ...
    let gap_count = indices
        .indices
        .windows(2)
        .filter(|window| window[1] != window[0] + 1)
        .count();
    gap_count <= (needle_len / 3).max(2)
}

/// Deterministic path/candidate fuzzy score without picker state or
/// frecency. `None` means the candidate does not match.
pub fn fuzzy_match_score(pattern: &str, candidate: &str) -> Option<u16> {
