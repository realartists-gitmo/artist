...
use gpui::{DmaBufSurface, PaintSurface};
use log::warn;
...
struct WgpuResources {
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    dma_bufs: HashMap<u64, ImportedDmaBuf>,
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    failed_dma_buf_presentations: HashSet<u64>,
}
...
impl WgpuResources {
    fn invalidate_intermediate_textures(&mut self) {
        ...
        {
            self.dma_bufs.clear();
            self.failed_dma_buf_presentations.clear();
        }
    }
}
...
impl WgpuRenderer {
    ...
    fn new_internal(
        ...
        let resources = WgpuResources {
            ...
            path_msaa_view: None,
            #[cfg(any(target_os = "linux", target_os = "freebsd"))]
            dma_bufs: HashMap::new(),
            #[cfg(any(target_os = "linux", target_os = "freebsd"))]
            failed_dma_buf_presentations: HashSet::new(),
        };
        ...
        loop {
                ...
                for batch in scene.batches() {
                    let ok = match batch {
                        ...
                        PrimitiveBatch::Surfaces(surfaces) => {
                            #[cfg(any(target_os = "linux", target_os = "freebsd"))]
                            {
                                self.draw_surfaces(&prepared_surfaces[surfaces], &mut pass)
                            }
            ...
            if overflow {
                ...
                if self.instance_buffer_capacity >= self.max_buffer_size {
                    ...
                    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
                    self.release_surfaces(&prepared_surfaces);
                    frame.present();
                    return true;
                }
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    fn prepare_surfaces(&mut self, scene: &Scene) -> Vec<Option<PreparedSurface>> {
        let live_images = scene
            .surfaces
            .iter()
            .map(|surface| surface.dma_buf.id())
            .collect::<HashSet<_>>();
        let live_presentations = scene
            .surfaces
            .iter()
            .filter_map(|surface| surface.dma_buf.presentation_id())
            .collect::<HashSet<_>>();
        {
            let resources = self.resources_mut();
            resources.dma_bufs.retain(|id, _| live_images.contains(id));
            resources
                .failed_dma_buf_presentations
                .retain(|id| live_presentations.contains(id));
        }

        let mut acquired_images = HashSet::new();
        scene
            .surfaces
            .iter()
            .map(|surface| {
                if surface
                    .dma_buf
                    .presentation_id()
                    .is_some_and(|id| self.resources().failed_dma_buf_presentations.contains(&id))
                {
                    return None;
                }

                let acquire = acquired_images.insert(surface.dma_buf.id());
                match self.prepare_surface(surface, acquire) {
                    Ok(surface) => Some(surface),
                    Err(error) => {
                        log::error!("failed to import GPUI DMA-BUF surface: {error:#}");
                        if let Some(presentation_id) = surface.dma_buf.presentation_id() {
                            self.resources_mut()
                                .failed_dma_buf_presentations
                                .insert(presentation_id);
                        }
                        surface.dma_buf.complete();
                        None
                    }
                }
            })
    ...
    fn prepare_surface(
        ...
        acquire: bool,
    ) -> anyhow::Result<PreparedSurface> {
        let id = surface.dma_buf.id();
        if !self.resources().dma_bufs.contains_key(&id) {
            let imported = Self::import_dma_buf(
                &self.resources().device,
                &self.resources().queue,
                &surface.dma_buf,
            )?;
            self.resources_mut().dma_bufs.insert(id, imported);
        } else if acquire {
            Self::transition_imported(
                &self.resources().device,
                &self.resources().queue,
                &self.resources().dma_bufs[&id].texture,
                true,
            )?;
        }

        let resources = self.resources();
        let imported = &resources.dma_bufs[&id];
    ...
    fn import_dma_buf(
    ...
    ) -> anyhow::Result<ImportedDmaBuf> {
        let [plane] = surface.planes() else {
            anyhow::bail!("only single-plane DMA-BUF surfaces are supported")
        };
        let format = drm_texture_format(surface.format())?;
        let descriptor = wgpu::TextureDescriptor {
            label: Some("gpui_dma_buf"),
            size: wgpu::Extent3d {
                width: surface.width(),
        ...
        let hal_device = unsafe { device.as_hal::<wgpu::hal::api::Vulkan>() }
            .ok_or_else(|| anyhow::anyhow!("the GPUI renderer is not using Vulkan"))?;
        let hal_queue = unsafe { queue.as_hal::<wgpu::hal::api::Vulkan>() }
            .ok_or_else(|| anyhow::anyhow!("the GPUI queue is not using Vulkan"))?;
        let fd = plane.fd().try_clone_to_owned()?;
        let hal_texture = unsafe {
            hal_device.texture_from_dma_buf(
                fd,
                surface.modifier(),
                plane.offset() as u64,
                plane.stride() as u64,
        ...
        unsafe {
            hal_device.transition_dma_buf_texture(&hal_queue, &hal_texture, true)?;
        }
        let texture = unsafe {
            device.create_texture_from_hal::<wgpu::hal::api::Vulkan>(hal_texture, &descriptor)
        };
        let view = texture.create_view(&wgpu::TextureViewDescriptor::default());
        log::debug!(
            "imported GPUI DMA-BUF image {} ({}x{}, format {:#x}, modifier {:#x})",
            surface.id(),
            surface.width(),
            surface.height(),
            surface.format(),
        ...
        Ok(ImportedDmaBuf { texture, view })
    }

    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    fn transition_imported(
        device: &wgpu::Device,
        queue: &wgpu::Queue,
        texture: &wgpu::Texture,
        acquire: bool,
        ...
        let hal_texture = unsafe { texture.as_hal::<wgpu::hal::api::Vulkan>() }
            .ok_or_else(|| anyhow::anyhow!("the GPUI texture is not Vulkan-backed"))?;
        unsafe { hal_device.transition_dma_buf_texture(&hal_queue, &hal_texture, acquire)? };
        Ok(())
    }

    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    fn release_surfaces(&self, surfaces: &[Option<PreparedSurface>]) {
        let mut released = HashSet::new();
        for prepared in surfaces.iter().flatten() {
            if !released.insert(prepared.source.id()) {
                continue;
            }
            if let Some(imported) = self.resources().dma_bufs.get(&prepared.source.id())
                && let Err(error) = Self::transition_imported(
                    &self.resources().device,
                    &self.resources().queue,
                    &imported.texture,
                    false,
            ...
            {
                log::error!("failed to release GPUI DMA-BUF surface: {error:#}");
            } else {
                log::debug!(
                    "completed GPUI DMA-BUF sample for image {}",
                    prepared.source.id()
                );
            }
        }
    ...
    fn draw_surfaces(
        &self,
        surfaces: &[Option<PreparedSurface>],
...
#[cfg(any(target_os = "linux", target_os = "freebsd"))]
fn drm_texture_format(format: u32) -> anyhow::Result<wgpu::TextureFormat> {
    match format {
        format
            if format == drm_fourcc(b'X', b'R', b'2', b'4')
                || format == drm_fourcc(b'A', b'R', b'2', b'4') =>
        {
            Ok(wgpu::TextureFormat::Bgra8Unorm)
        }
        format
            if format == drm_fourcc(b'X', b'B', b'2', b'4')
                || format == drm_fourcc(b'A', b'B', b'2', b'4') =>
        {
            Ok(wgpu::TextureFormat::Rgba8Unorm)
        }
        format => anyhow::bail!("unsupported DRM FourCC {format:#010x}"),
    }
}
