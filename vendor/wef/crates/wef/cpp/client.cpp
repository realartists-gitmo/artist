...
void WefClient::OnAccessibilityTreeChange(CefRefPtr<CefValue> value) {
  auto json = CefWriteJSON(value, JSON_WRITER_DEFAULT).ToString();
  state_->callbacks_target.call(
      [&](const BrowserCallbacks& callbacks, void* userdata) {
        callbacks.on_accessibility_update(userdata, false, json.c_str());
      });
}

void WefClient::OnAccessibilityLocationChange(CefRefPtr<CefValue> value) {
  auto json = CefWriteJSON(value, JSON_WRITER_DEFAULT).ToString();
  state_->callbacks_target.call(
      [&](const BrowserCallbacks& callbacks, void* userdata) {
        callbacks.on_accessibility_update(userdata, true, json.c_str());
      });
}
