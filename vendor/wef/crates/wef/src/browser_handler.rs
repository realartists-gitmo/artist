...
pub trait BrowserHandler {
    ...
    /// Chromium accessibility tree or location update in CEF's JSON shape.
    fn on_accessibility_update(&mut self, locations_only: bool, update: serde_json::Value) {}
}

impl BrowserHandler for () {}

pub(crate) extern "C" fn on_accessibility_update<T: BrowserHandler>(
    userdata: *mut c_void,
    locations_only: bool,
    json: *const c_char,
) {
    unsafe {
        ...
        if let Ok(update) = serde_json::from_str(&json) {
            state
                .handler
                .on_accessibility_update(locations_only, update);
        }
    }
}
