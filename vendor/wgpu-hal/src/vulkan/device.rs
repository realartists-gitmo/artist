...
impl super::Device {
    ...
    fn create_image_without_memory(
        ...
        let mut vk_info = vk::ImageCreateInfo::default()
            ...
            .tiling(if drm_modifier_info.is_some() {
        ...
        if let Some(modifier_info) = drm_modifier_info {
            vk_info = vk_info.push_next(modifier_info);
        }
    ...
    pub unsafe fn texture_from_d3d11_shared_handle(
        ...
        let image =
            self.create_image_without_memory(desc, Some(&mut external_memory_image_info), None)?;
    ...
    /// Import a single-plane Linux DMA-BUF as a sampled Vulkan texture.
    ///
    /// The file descriptor is consumed by Vulkan on successful allocation. The
    /// descriptor must name memory compatible with `desc`, `modifier`, and the
    /// explicit plane layout.
    ///
    /// # Safety
    ///
    /// The DMA-BUF must remain exclusively synchronized by the caller until
    /// GPU work using the returned texture completes. Its dimensions, format,
    /// modifier, offset, and stride must match the allocation exactly.
    #[cfg(unix)]
    pub unsafe fn texture_from_dma_buf(
        &self,
        fd: OwnedFd,
        modifier: u64,
        offset: u64,
        ...
        for extension in [
            ...
            if !self.shared.enabled_extensions.contains(&extension) {
                return Err(crate::DeviceError::Unexpected);
            }
        }
        ...
        let image = self.create_image_without_memory(
            desc,
            Some(&mut external_info),
            Some(&mut modifier_info),
        )?;

        let external_memory_fd =
            khr::external_memory_fd::Device::new(&self.shared.instance.raw, &self.shared.raw);
        ...
        if let Err(error) = unsafe {
            ...
            return Err(super::map_host_device_oom_err(error));
        }

        let compatible_memory_types =
            image.requirements.memory_type_bits & fd_properties.memory_type_bits;
        ...
        let mut dedicated = vk::MemoryDedicatedAllocateInfo::default().image(image.raw);
        ...
        let memory = match unsafe { self.shared.raw.allocate_memory(&allocation, None) } {
            Ok(memory) => memory,
            Err(error) => {
                unsafe {
        ...
        if let Err(error) = unsafe { self.shared.raw.bind_image_memory(image.raw, memory, 0) } {
            unsafe {
    ...
    /// Acquire an imported DMA-BUF from a foreign producer and make it
    /// shader-readable on this device queue, or release it back to the foreign
    /// producer after rendering.
    ///
    /// This operation waits for the queue because the compositor's existing
    /// double-buffer contract has no explicit sync-fence channel yet.
    ///
    /// # Safety
    ///
    /// `texture` must be an image imported by [`Device::texture_from_dma_buf`]
    /// and `queue` must belong to this device.
    #[cfg(unix)]
    pub unsafe fn transition_dma_buf_texture(
        &self,
        queue: &super::Queue,
        ...
        let command = match unsafe { self.shared.raw.allocate_command_buffers(&allocation) } {
        ...
        if let Err(error) = unsafe { self.shared.raw.begin_command_buffer(command, &begin) } {
            unsafe { self.shared.raw.destroy_command_pool(pool, None) };
            return Err(super::map_host_device_oom_err(error));
        }
        ...
        if let Err(error) = unsafe { self.shared.raw.end_command_buffer(command) } {
            unsafe { self.shared.raw.destroy_command_pool(pool, None) };
            return Err(super::map_host_device_oom_err(error));
        }
        ...
        let result = unsafe {
            self.shared
                .raw
                .queue_submit(queue.as_raw(), &[submit], vk::Fence::null())
                .and_then(|_| self.shared.raw.queue_wait_idle(queue.as_raw()))
        };
        unsafe { self.shared.raw.destroy_command_pool(pool, None) };
        result.map_err(super::map_host_device_oom_err)
    }
...
impl crate::Device for super::Device {
    ...
    unsafe fn create_texture(
        ...
        let image = self.create_image_without_memory(desc, None, None)?;
